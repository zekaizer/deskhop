...
/// Iterative CRC32 step
pub fn crc32_iter(crc: u32, byte: u8) -> u32 {
    CRC32_TABLE[((byte as u32 ^ crc) & 0xff) as usize] ^ (crc >> 8)
}
...
mod tests {
    ...
    #[test]
    fn test_calc_checksum_empty() {
        assert_eq!(calc_checksum(&[]), 0);
    }

    #[test]
    fn test_calc_checksum_single() {
        assert_eq!(calc_checksum(&[0x42]), 0x42);
    }

    #[test]
    fn test_calc_checksum_xor() {
        assert_eq!(calc_checksum(&[0xFF, 0xFF]), 0x00);
        assert_eq!(calc_checksum(&[0xAA, 0x55]), 0xFF);
    }
    ...
    fn test_crc32_known_value() {
        ...
        assert_eq!(calc_crc32(data), 0xCBF43926);
    }

    #[test]
    fn test_crc32_empty() {
        assert_eq!(calc_crc32(&[]), 0x00000000);
    }

    #[test]
    fn test_crc32_single_byte() {
        assert_eq!(calc_crc32(&[0x00]), 0xD202EF8D);
    }

    #[test]
    fn test_crc32_iter_matches_full() {
        let data = b"test";
        let mut crc: u32 = 0xffff_ffff;
        for &b in data.iter() {
