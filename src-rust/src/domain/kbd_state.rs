...
pub fn update_kbd_state(state: &mut DeviceState<'_>, report: &HidKeyboardReport, device_idx: u8) {
    let idx = device_idx as usize;
    if idx >= MAX_DEVICES {
        return;
    }
    state.hid.local_kbd_states[idx] = *report;
    if state.hid.max_kbd_idx < device_idx {
        state.hid.max_kbd_idx = device_idx;
    }
}

/// Update remote keyboard state (from other board via UART)
pub fn update_remote_kbd_state(state: &mut DeviceState<'_>, report: &HidKeyboardReport) {
    state.hid.remote_kbd_state = *report;
}
...
pub fn combine_kbd_states(state: &DeviceState<'_>) -> HidKeyboardReport {
    let mut combined = HidKeyboardReport::default();

    for i in 0..=(state.hid.max_kbd_idx as usize) {
        if i >= MAX_DEVICES {
            break;
        }
        combined.modifier |= state.hid.local_kbd_states[i].modifier;
        add_keys(&mut combined, &state.hid.local_kbd_states[i]);
    }

    combined.modifier |= state.hid.remote_kbd_state.modifier;
    add_keys(&mut combined, &state.hid.remote_kbd_state);

    combined
}

/// Release all keys — clear all keyboard states. Caller must queue empty report.
pub fn release_all_keys(state: &mut DeviceState<'_>) {
    for i in 0..MAX_DEVICES {
        state.hid.local_kbd_states[i] = HidKeyboardReport::default();
    }
    state.hid.remote_kbd_state = HidKeyboardReport::default();
}
...
mod tests {
    ...
    fn test_update_kbd_state() {
        ...
        assert_eq!(state.hid.max_kbd_idx, 0);

        update_kbd_state(&mut state, &report, 2);
        assert_eq!(state.hid.max_kbd_idx, 2);
    }
    ...
    fn test_combine_kbd_states() {
        ...
        state.hid.local_kbd_states[1] = HidKeyboardReport {
            modifier: 0x02, reserved: 0, keycode: [0x05, 0, 0, 0, 0, 0],
        };
        state.hid.max_kbd_idx = 1;
    ...
    fn test_release_all_keys() {
        ...
        state.hid.remote_kbd_state.modifier = 0x02;

        release_all_keys(&mut state);

        assert_eq!(state.hid.local_kbd_states[0].modifier, 0);
        assert_eq!(state.hid.local_kbd_states[0].keycode[0], 0);
