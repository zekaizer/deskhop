...
pub trait ReportRouter: ReportQueue + PacketQueue + PeerLink + Timer {
    fn route_mouse(&self, state: &mut DeviceState<'_>, report: &[u8]) {
    ...
    fn route_kbd(&self, state: &mut DeviceState<'_>, report: &[u8]) {
        if state.is_active_output() {
    ...
    fn route_consumer(&self, state: &mut DeviceState<'_>, report: &[u8]) {
        if state.is_active_output() {
    ...
    fn route_system(&self, state: &mut DeviceState<'_>, report: &[u8]) {
        if state.is_active_output() {
    ...
    fn touch_activity(&self, state: &mut DeviceState<'_>) {
        let role = state.cfg.board_role as usize;
        if role < state.cfg.last_activity.len() {
            state.cfg.last_activity[role] = self.now_us_64();
        }
    }
}
...
mod tests {
    ...
    fn route_mouse_inactive_output() {
        ...
        let mut state = DeviceState { hid: &mut hid, cfg: &mut cfg, fw: &mut fw, led: &mut led };
        state.cfg.board_role = 0;
        state.cfg.active_output = 1; // inactive
        ...
        assert_eq!(state.cfg.last_activity[0], 0); // no timestamp update
    }
    ...
    fn touch_activity_updates_timestamp() {
        ...
        assert_eq!(state.cfg.last_activity[1], 5_000_000);
        assert_eq!(state.cfg.last_activity[0], 0); // other role untouched
    }
}
