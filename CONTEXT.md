...
- `PT_STATE` (passthrough state singleton in `src-rust/src/hal/ffi/tasks.rs`) **violates ADR-0002's "no contention" invariant** for cross-core data. It is mutated from both Core0 (passthrough_task scheduler, `rust_on_tud_set_report`) and Core1 (`rust_on_hid_mount`/`umount`/`report_received` TinyUSB host callbacks), and read from Core0 USB device descriptor callbacks — all without atomic protection. Failure modes: torn descriptor reads during enumeration, use-after-compaction when `remove_device` runs concurrently with descriptor send. Race windows are narrow in practice but real. Production fix is a Core1→Core0 event queue (mount/umount/report) so all PT_STATE mutation is serialized to Core0; deferred. The comment block above `PT_STATE` in `hal/ffi/tasks.rs` is the source of truth on access boundaries.
